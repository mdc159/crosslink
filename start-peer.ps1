...
# Start task worker in separate window
Write-Host "Starting task worker..."
Start-Process powershell -ArgumentList "-NoExit", "-Command", "Set-Location '$ScriptDir'; .\scripts\crosslink-worker.ps1"

# Start stats collector in background (silent)
Write-Host "Starting stats collector..."
Start-Process powershell -ArgumentList "-WindowStyle", "Hidden", "-Command", "Set-Location '$ScriptDir'; .\scripts\windows-collector.ps1" -WindowStyle Hidden
...
Write-Host "    - Task worker (shows incoming tasks)"
Write-Host "    - Stats collector (hidden, sending data)"
Write-Host ""
